...
interface ThinkingSpinner3DProps {
  size?: SpinnerSize;
...
export function ThinkingSpinner3D({
  size = 'sm',
  variant = 'globe',
  renderMode = 'webgl',
  className = '',
  ...
  if (renderMode === 'ascii') {
    return (
      <span
        className={`thinking-spinner-3d thinking-spinner-3d--${size} ${className}`}
        role="status"
        aria-label="loading"
