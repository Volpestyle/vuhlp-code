...
function AutoRotateScene({ children, speed = 1.0 }: { children: React.ReactNode; speed?: number }) {
...
const shapeScenes: Record<ShapeType, React.FC> = {
...
export function AsciiSpinner({
  ...
  if (customScene) {
    ...
    Content = (
      <React.Suspense fallback={<SpinnerScene />}>
    ...
    const currentShape = activeShapes[index] || 'cube';
  ...
  return (
    <View style={{
      ...
      // Adding native transition support isn't straightforward in plain RN/View style, 
      // but opacity state handles the visual fade.
      ...style
    }}>
      <AsciiRenderer
        cols={settings.cols}
